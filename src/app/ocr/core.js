...
export default class LensCore {
  ...
  async fetchData(formdata, originalDimensions, secondTry = false) {
    ...
    params.append('ep', 'gsbubb'); // EntryPoint
    ...
    if (response.status === 302) {
      ...
      if (saveConsentRequest.status === 303) {
        ...
        await sleep(500);
        return this.fetchData(formdata, originalDimensions, true);
      }
    }
  ...
  async scanByData(uint8, mime, originalDimensions) {
    ...
    return this.fetchData(formdata, originalDimensions);
  }
  ...
  #setCookies(combinedCookieHeader) {
    if (combinedCookieHeader) {
      const splitCookieHeaders = combinedCookieHeader.split(/,(?=[^;]*\bexpires\b)/);
      const cookies = this.#parseCookiesString(splitCookieHeaders);
  ...
  #parseCookiesString(cookieStrings) {
    return cookieStrings.map(cookieString => {
      ...
      if (!cookie.expires) {
        cookie.expires = new Date(Date.now() + 24 * 60 * 60 * 1000); // default to 1 day if no expiration is set
      }
